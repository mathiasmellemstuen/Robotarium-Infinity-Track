...
Because of this, we only need an algorithm that calculates vertical and algular velocity of the robot so the robot moves towards the current waypoint. When the robot then has approached a acceptable distance the the current waypoint, then change to the next waypoint in the path and run the algorithm again. 
